...
public class AddPlanServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Integer New_Plan_ID = Plan.InsertPlan(Plan);
            
            if(New_Plan_ID != null && New_Plan_ID > 0) {
                String successUrl = contextPath + "/Messages/PlanInsertSuccess.html?planid=" + New_Plan_ID 
                    + "&planname=" + URLEncoder.encode(planname, "UTF-8")
                    + "&monthlyfee=" + monthlyfee;
                response.sendRedirect(successUrl);
            } else {
                String errorUrl = contextPath + "/Messages/PlanInsertFailed.html?message=" 
                    + URLEncoder.encode("Failed to insert plan. Please check the data.", "UTF-8");
